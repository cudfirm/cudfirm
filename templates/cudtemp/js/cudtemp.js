  ...
  link.addEventListener('click', e => {
    ...
    if (pricingToggle && monthlyLabel && annualLabel) {
      ...
      const updateDeliveryMode = () => {
        monthlyLabel.classList.toggle('active', !cmsIncluded);
        annualLabel.classList.toggle('active', cmsIncluded);
        pricingToggle.classList.toggle('annual', cmsIncluded);
        pricingToggle.setAttribute('aria-checked', String(cmsIncluded));
      };
      const toggleDeliveryMode = () => { cmsIncluded = !cmsIncluded; updateDeliveryMode(); };
      pricingToggle.addEventListener('click', toggleDeliveryMode);
      pricingToggle.addEventListener('keydown', event => {
        if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggleDeliveryMode(); }
      });
      updateDeliveryMode();
    }
